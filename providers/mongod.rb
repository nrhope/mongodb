...
#    end 
    ...
    service instance_name do
      ...
      reload_command "#{node['runit']['sv_bin']} hup #{node['runit']['service_dir']}/#{instance_name}"
    end

    execute "check #{instance_name} service is running" do
      action :run
      command "sv status #{instance_name}"
    end
  end 
